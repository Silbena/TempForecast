...
def arima(df):
    model = ARIMA(df, order=(2, 0, 24)).fit()
...
def plot_arima(past_df, pred_mean, conf_ints):
    ...
    base = px.line(past_df, x = past_df.index, y = 'AverageTemperatureCelsius',
                  title = 'Average Temperature Forecast for Sweden (ARIMA)',
                  labels = {'x':'Year', 'y':'Temperature [C]'}).data[0]
